/**
 * 奇门遁甲各元素含义解释
 * 包含宫位、八门、九星、八神、天干、地支等详细含义
 */

export const palaceMeanings: Record<string, string> = {
  // ============ 八宫含义 ============
  ...
  '天辅': '原名文曲星，与东南四宫巽卦相对应，阳星，五行属木。人们认为它是天上的文曲星，与文化教育事业有关，故称为大吉之星。\n\n有才华，攻于心计，有心眼，能屈能伸，管理型人员。指医生，医药。死门见心，出医生。',
  '天禽': '原名廉贞星，位于中央五宫，阴星，五行属土。天禽居中央，为诸星之枢纽，可以寄于坤二宫。',
  ...
  '天任': '原名左辅星，与东北八宫艮卦相对应，阳星，五行属土。人们认为土能生万物，又正当春季万物萌生之时，故称之为吉星。\n\n测合作，一方临蛇，不可信，也代表事物的牵连。测病，易神经出问题。',
  '天英': '原名右弼星，与南方九宫离卦相对应，阴星，五行属火。人们认为天英星居离宫之位，烈火炎炎，性躁易暴，虽然如日升中天，在放光明，但又和血光之灾有关，故称为中平之星，或小凶之星。\n\n生旺，有名声，有名利。测终生，天英旺有名。落艮宫不吉，有名不想出名。落坤宫，全国出名，因坤主大地，主小百姓。急躁、暴躁。配合不当出坏名声，易发火，没有沉稳性。',
  '禽芮': '天禽与天芮同宫，天禽寄坤二宫时与天芮一起出现。兼具天禽枢纽之能与天芮病星之性。',
  ...
  '九地': '坤土之象，万物之母。为坚牢之神，性柔好静。九地之方，可以屯兵固守。\n\n代表稳固、保守、坚守阵地。利于防守、储藏、隐藏。与土地、房产、母亲有关。',
  ...
  '庚': '庚为阳金，名为剑戟之金，有刚健肃杀之力，其性刚质硬。主人形体瘦长，骨格健壮，长脸白皮肤，筋强骨健。为人性情刚健敏锐、有威严、坚忍不拔、能屈人而不屈于人。奇门遁甲中称庚为天狱为天刑，代表仇人、丈夫、公安干警、道路、皮毛、大肠、骨骼、肺、大腿骼、道路、阻隔、阻挡、技术、虎、军人、背、打斗、刀、武器、佛、高个子体态长方、钢键锐力、威严、盆腔、收费站、脊骨、关口、黑社会、车祸、石块、墙角、技术过硬、凶猛等。',
  ...
  '癸': '癸为阴水，名为雨露之水，性发生至静至弱，滋润万物。主人矮小黑丑，圆脸瘦肩，声调不高，为人性情阴柔怕事，多伤感，不能自主。奇门遁甲中称癸为天网，代表性及性生活有关之事、牢狱、小水、淋浴、饮料、咖啡、可乐、眼泪、哭泣、奶、尿尿、厕所、(癸加辛水珠)汤粥、污水、眼睛(水汪汪的大眼睛)、湿度大的雾气、(戊加癸是钱财都流了)洗浴用品、鞋子、头发、油类、鱼缸、下雨、子宫、足、女性用品、便池、鼻涕、蛋、黑色的发、黑色的眼睛、困难、足、静脉、血液、酒、(癸加己酒鬼，丁加己烟鬼，在坎宫女人来例假)色情、肾、神经系统、化妆品、小孩子、精液。',
...
/**
 * 获取元素含义
 * @param key 元素名称（宫位、八门、九星、八神、天干、地支等）
 * @returns 含义解释文字
 */
export function getMeaning(key: string): string | undefined {
  return palaceMeanings[key];
}

/**
 * 获取宫位含义
 * @param gongName 宫位名称（乾、坎、艮、震、巽、离、坤、兑、中）
 * @returns 宫位含义
 */
export function getGongMeaning(gongName: string): string | undefined {
  return palaceMeanings[gongName + '宫'];
}
 ...
 * @returns 八门含义
 */
export function getMenMeaning(menName: string): string | undefined {
  return palaceMeanings[menName];
}
 ...
 * @returns 九星含义
 */
export function getXingMeaning(xingName: string): string | undefined {
  return palaceMeanings[xingName];
}
 ...
 * @returns 八神含义
 */
export function getShenMeaning(shenName: string): string | undefined {
  return palaceMeanings[shenName];
}
 ...
 * @returns 天干含义
 */
export function getGanMeaning(ganName: string): string | undefined {
  return palaceMeanings[ganName];
}
 ...
 * @returns 地支含义
 */
export function getZhiMeaning(zhiName: string): string | undefined {
